...
- **Step 3 — client + pages/SSR on Vite.** Move page-module loading + SSR to the runner (unify `reloadPages` with `runner.import`); analyzer becomes a Vite plugin; Vite's HMR ws replaces the SSE live-reload; Fast Refresh for pages. Remove the rolldown **dev** page build (prod build stays). **Sub-stages:**
  ...
  - **3c — retire the Tier-0 SSE reload; Vite owns all browser reloads.** ✅ Removed the SSE reload server (`dev-reload-server.ts` + its unit test), the `reload.notify()` calls, `PYLON_DEV_RELOAD_PORT` (→ a plain `PYLON_DEV` flag for the SSR-minify check), and the dead SSE `EventSource` injection in the rolldown client. Browser reloads now go entirely through Vite: **Fast Refresh** for component edits, and a **Vite ws full-reload** on `src`/resolver edits (the dev worker calls `bridge.reloadBrowser()` after the schema hot-swap so pages re-fetch). Validated on docs (component edit → in-place; src edit → full-reload + re-hydrate) + `dev-pages-loop` 6/6 (Tier-0 test dropped). **Also retired the redundant dev client build**: the rolldown *client* JS bundle is dead in dev (Vite serves the client), but its CSS output feeds the SSR `<link precedence>` (no-FOUC). The server build already traverses the same graph + collects CSS, so in dev (`PYLON_DEV`) it now writes the CSS + a CSS-only static manifest (`index.css`/`app.css`/`version`, no `app.js`) and the client build is SKIPPED — one rolldown build per dev rebuild instead of two. Prod (`pylon build`) still runs both (client `app.js` + server). Validated: docs styled first paint + `Pages [client]` build gone in dev / present in prod; `dev-pages-loop` 6/6.

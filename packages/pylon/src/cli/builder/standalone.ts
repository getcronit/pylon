...
export interface StandaloneResult {
  ...
  outDir: string
  /** The dir to run FROM (cwd) — the app's base-relative location inside the output. The
   *  app resolves `.pylon/**` relative to cwd, so the launcher `chdir`s here first. */
  runDir: string
  /** Absolute path to the generated launcher — run it from ANY cwd (`node <launcher>`). */
  launcher: string
  /** Absolute path to the server entry itself (the launcher chdir's + imports it). */
  entry: string
...
export async function buildStandalone(opts: {
  ...
  // The app resolves `.pylon/**` relative to cwd, so the run dir is the app's base-relative
  // location inside the output (the output root for a lone app; `<output>/<pkg-path>` in a
  // monorepo). A generated launcher chdir's there and imports the server entry, so the
  ...
  await fs.promises.writeFile(
    launcher,
    [
      `// GENERATED by \`pylon build --standalone\` — do not edit. Runs the traced app from`,
      `// any cwd: chdir into the app dir (so \`.pylon/**\` reads resolve) then import the entry.`,
      `import path from 'node:path'`,
      `import {fileURLToPath, pathToFileURL} from 'node:url'`,
      `const here = path.dirname(fileURLToPath(import.meta.url))`,
      `const runDir = path.join(here, ${JSON.stringify(appRel)})`,

...
export async function buildStandalone(opts: {
  ...
  // Stable ROOT launchers — runnable from any cwd, so callers never need the app's nested
  // `<appRel>/.pylon/*.mjs` path. Each chdir's into the app dir (so the app's OWN cwd-relative
  // reads like a `content/` dir resolve) then imports its entry; the FRAMEWORK anchors to the
  // entry location (the entry sets __PYLON_ROOT__) and needs no chdir, so the nested entry is
  // also runnable directly. `start.mjs` serves; `start-worker.mjs` consumes queues (no HTTP).
  const writeLauncher = async (file: string, entryFile: string, note: string): Promise<void> => {
    await fs.promises.writeFile(
      path.join(standaloneDir, file),
      [
        `// GENERATED by \`pylon build --standalone\` — do not edit. ${note}`,
        `// Also runnable directly: node ${path.posix.join(appRel || '.', '.pylon', entryFile)}`,
        `import path from 'node:path'`,
        `import {fileURLToPath, pathToFileURL} from 'node:url'`,
        `const runDir = path.join(path.dirname(fileURLToPath(import.meta.url)), ${JSON.stringify(appRel)})`,
        `process.chdir(runDir)`,
        `await import(pathToFileURL(path.join(runDir, '.pylon', ${JSON.stringify(entryFile)})).href)`,
        ``
      ].join('\n')
    )
  }
  ...
  await writeLauncher('start.mjs', 'server.mjs', 'The web entry (`node start.mjs`) — serves HTTP.')
  let launcherCount = 1
  // The worker entry is emitted whenever the app builds; give it a symmetric launcher so the
  // worker is `node start-worker.mjs`, mirroring `node .pylon/worker.mjs` in a non-traced build.
  if (fs.existsSync(workerEntry)) {
    await writeLauncher(
      'start-worker.mjs',
      'worker.mjs',
      'The background-worker entry (`node start-worker.mjs`) — consumes queues + drains the outbox, no HTTP.'
    )
    launcherCount += 1
  }
  ...
  return {
    ...
    launcher,
    entry,
    fileCount: fileCount + launcherCount,
    byteCount,
    ...(() => {

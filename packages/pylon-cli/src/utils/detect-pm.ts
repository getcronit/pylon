...
async function hasGlobalInstallation(pm: PM): Promise<boolean> {
  ...
  try {
    const proc = Bun.spawn([pm, '--version'])

    const value = await new Response(proc.stdout).text().catch(() => '')

    const isAvailable = /^\d+.\d+.\d+$/.test(value)
...
export async function getNpmVersion(pm: PM) {
  const proc = Bun.spawn([pm || 'npm', '--version'])
...
console.log(await detect({cwd: process.cwd()}))

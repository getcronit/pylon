    ...
    async setup(build) {
      build.onLoad({filter: /.*/, namespace: 'file'}, async args => {
        ...
        if (isEntryPoint) {
          ...
          // Override the defaultExport with the PylonPageLoader
          contents = contents.replace(
            `export default ${defaultExport}`,
            `const {PylonPageLoader} = await import("@getcronit/pylon/page-loader.js")
export default <PylonPageLoader pagePath={"${buildPath}"} client={client} Element={${defaultExport}} cacheSnapshot={window.__pylon_cache_snapshot} />`
          )
          ...
          return {
            ...
            if(typeof window !== 'undefined') {
               ...
               const client = await import("${pathToClient}")
                ...
                <PylonPageLoader pagePath={"${buildPath}"} client={client} Element={${defaultExport}} cacheSnapshot={cacheSnapshot} />
                )

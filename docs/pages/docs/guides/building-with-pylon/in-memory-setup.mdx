...
The easiest way to get started with Pylon is to use in-memory data. For this, we will create a simple class that holds the data and performs all the necessary operations on it.
...
Since Pylon let's us freely implement our logic, independent of the structure and its location, we can simply create a new file directly in the `src/` directory:
...
Currently, the class only holds the static data. In the next step, we will add methods to interact with the data.
...
And that's it! We have successfully implemented a simple class that holds static data and provides methods to interact with it. To test the implementation, Pylon provides a built-in [GraphQL playground](/docs/getting-started#built-in-graphql-playground) where we can execute our new queries:

![Pylon GraphQL Query](/images/guides/building-with-pylon/static-playground-query.png)
...
This method is pretty straightforward. It takes a new booking object, assigns a new ID to it, and adds it to the `bookings` array. Since we dont want the client to set the ID, we use `Omit<Booking, "id">` to exclude the `id` property from the `booking` object.

To make the new method available in our service, we need to add it to the `index.ts` file (dont forget to bind `this` to the method):
...
![Pylon GraphQL Mutation](/images/guides/building-with-pylon/static-playground-add.png)
...
This guide showed how easy it is to get started with Pylon. We created a simple class that holds in-memory data and provides methods to interact with it. We then registered the class methods in Pylon and tested them using the built-in GraphQL playground.

In the next guide, we will take a look at how we can use external data sources to fetch and modify data in our service.

...
We will start with a simple setup using [hardcoded data](./hardcoded-data.mdx) and then move on to communicate with an external API ([External API](./external-api)) and finally use Prisma to interact with a database ([Integrating Prisma](./integrating-prisma)).
